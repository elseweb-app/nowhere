...
§15) — nothing new needed at the protocol level for that. It is also the intended
home for a **compute bridge**: talking to a local OpenAI-compatible endpoint (Ollama
and similar) so a WebGPU/Ollama-style compute provider can be reached over the
protocol. That bridge has no protocol-level design yet — do not build ahead of a spec
for it; treat it as a direction, not a committed interface.
